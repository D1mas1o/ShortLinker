...
@app.route('/links<short_link>',methods=['POST'])
def delete_link(short_link):
    ...
    del_link(data,short_link)
    return redirect(url_for('user_links'))
...
def authenticate():
    ...
    {'WWW-Authenticate': 'Basic realm="Login Required" Basic-cookie="None"'})
################################
...
app.add_url_rule('/links',view_func=UserLinks.as_view('user_links'))
app.add_url_rule('/',view_func=UserIndex.as_view('index'))

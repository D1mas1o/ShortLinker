...
def short_answer(short_url):
    with sqlite3.connect('ShortLinkBd.db') as conn:
        c = conn.cursor()
        c.execute('SELECT LongLink,LinkMode,UserId,ShortLink FROM Links WHERE ShortLink=(?)',(short_url,))
        ...
        author = mas[2]
        short_link = mas[3]
    return long_link,link_mode,author,short_link
...
def check_base_auth_private(username, password,author):
    with sqlite3.connect('ShortLinkBd.db') as conn:
        c = conn.cursor()
        c.execute('SELECT Login,Password FROM Users WHERE UserId=(?)', (author,))
        author_info = c.fetchall()[0]
        password = hashlib.sha1(pickle.dumps(password))
        password = password.hexdigest()
        print(password)
        if username==author_info[0] and password==author_info[1]:
            return True
    return False
...
def bd_search_links(data):
    with sqlite3.connect('ShortLinkBd.db') as conn:
        ...
        mas = c.fetchall()
    return mas
def del_link(data,short_link):
    try:
        with sqlite3.connect('ShortLinkBd.db') as conn:
            c = conn.cursor()
            c.execute('DELETE FROM Links WHERE ShortLink=(?) AND UserId=(?)',(short_link,data['UserId']))
    except requests.exceptions.MissingSchema:
        pass
def add_counter_bd(counter,info):
    try:
        with sqlite3.connect('ShortLinkBd.db') as conn:
            c = conn.cursor()
            c.execute('UPDATE Links SET Counter=(?) WHERE ShortLink=(?)',(counter,info[3]))
    except requests.exceptions.MissingSchema:
        pass